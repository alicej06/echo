...
export default function DemoScreen() {
  ...
  const [predictions, setPredictions] = useState<Array<{ letter: string; conf: number }>>([]);
  ...
  useEffect(() => {
    ...
    fetch(`${cfg.serverUrl}/health`, { signal: AbortSignal.timeout(5000) })
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.serverRow}>
        <View style={[styles.dot, { backgroundColor: serverStatus === 'ok' ? '#4CAF50' : serverStatus === 'error' ? '#f44336' : '#888' }]} />
        <Text style={styles.serverText}>
          {serverStatus === 'ok' ? 'Railway server online' : serverStatus === 'error' ? 'Server offline - check Settings' : 'Checking server...'}
        </Text>
      </View>
      ...
      <View style={styles.card}>
        ...
        >
          ...
          <Text style={styles.demoBtnText}>{running ? 'Stop Demo' : 'Start Demo'}</Text>
        </Pressable>
      </View>
        ...
        <View style={styles.card}>
          ...
          {predictions.map((p, i) => (
            <View key={i} style={styles.predRow}>
              <Text style={[styles.predLetter, { opacity: 1 - i * 0.08 }]}>{p.letter}</Text>
              <View style={styles.barBg}>
                <View style={[styles.barFill, { width: `${p.conf * 100}%` }]} />
              </View>
              <Text style={[styles.predConf, { opacity: 1 - i * 0.08 }]}>{(p.conf * 100).toFixed(0)}%</Text>
            </View>
          ))}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  cardSubtitle: { color: '#888', fontSize: 13, marginBottom: 20, lineHeight: 18 },
  bigLetter: {
    fontSize: 120, fontWeight: '900', color: '#fff',
    textAlign: 'center', lineHeight: 140,
  },
  demoBtn: {
  ...
  barBg: {
    flex: 1, height: 6, backgroundColor: '#333',
